...
namespace Game.Logic.PetEffects.Element.Actives
{
	public class AE1546 : BasePetEffect
	{
		...
		{
			AE1546 aE = living.PetEffectList.GetOfType(ePetEffectType.AE1546) as AE1546;
			if (aE == null)
			{
		...
		private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
		{
			if (IsTrigger)
			{
				...
				new CE1546(2, m_probability, m_type, m_currentId, m_delay, base.ElementInfo.ID.ToString()).Start(target);
			}
		}
