	...
	public class AE1432 : BasePetEffect
	{
		...
		private void player_AfterBuffSkillPetByLiving(Player player)
		{
			if (player.PetEffects.CurrentUseSkill == m_currentId && player.Game is PVPGame)
			{
				...
				{
					if (allEnemyPlayer.IsHide)
					{
						player.Game.SendMarkMeHideInfo(player, allEnemyPlayer.Id, isMark: true);
						allEnemyPlayer.AddPetEffect(new CE1432(2, m_probability, m_type, m_currentId, m_delay, base.ElementInfo.ID.ToString()), 0);
					}
				}
			}
		}
