package pet.sprite
{
   ...
   public class PetSpriteController extends EventDispatcher
   {
      ...
      private function _onFunc(param1:FrameEvent) : void
      {
         SoundManager.instance.play("008");
         var _loc2_:BaseAlerFrame = param1.currentTarget as BaseAlerFrame;
         _loc2_.removeEventListener(FrameEvent.RESPONSE,this._onFunc);
      ...
      public function hidePetSprite(param1:Boolean = false, param2:Boolean = true) : void
      {
         ...
         var alert11:BaseAlerFrame = null;
         var immediately:Boolean = param1;
         var canShowNext:Boolean = param2;
         try
         {
            if(!this._petSprite)
            {
         ...
         catch(e:Error)
         {
            ...
            if(_loopTimer)
            {
               str1 = str1 + ("_loopTimer:" + _loopTimer.toString() + "---\n");
            }
            if(_petSprite)
            {
               str1 = str1 + ("_petSprite:" + _petSprite.toString() + "---\n");
            }
            if(immediately)
            {
               str1 = str1 + ("immediately:" + immediately.toString() + "---\n");
            }
            if(canShowNext)
            {
               str1 = str1 + ("canShowNext:" + canShowNext.toString() + "---\n");
            }
            if(_hasBeenSetup)
            {
               str1 = str1 + ("_hasBeenSetup:" + _hasBeenSetup.toString() + "---\n");
            }
            alert11 = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("AlertDialog.Info"),str1 + "+" + e.name + "+" + e.message,LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"));
            alert11.addEventListener(FrameEvent.RESPONSE,_onFunc);
            return;
         }
      }
