	...
	public class ConsortiaBussiness : BaseBussiness
	{
		...
		public ConsortiaAllyInfo[] GetConsortiaAllyPage(int page, int size, ref int total, int order, int consortiaID, int state, string name)
		{
			...
			try
			{
				if (state != -1)
				{
					...
					str3 += 0;
					queryWhere = ((state != 0) ? (queryWhere + " and ConsortiaID in (" + str3 + ") ") : (queryWhere + " and ConsortiaID not in (" + str3 + ") "));
				}
		...
		public ConsortiaApplyAllyInfo[] GetConsortiaApplyAllyPage(int page, int size, ref int total, int order, int consortiaID, int applyID, int state)
		{
			...
			try
			{
				...
				string fdOreder2 = "ConsortiaName";
				switch (order)
				{
				case 1:
					fdOreder2 = "Repute";
				...
				fdOreder2 += ",ID ";
				foreach (DataRow row in GetPage("V_Consortia_Apply_Ally", queryWhere, page, size, "*", fdOreder2, "ID", ref total).Rows)
				{
		...
		public ConsortiaDutyInfo[] GetConsortiaDutyPage(int page, int size, ref int total, int order, int consortiaID, int dutyID)
		{
			...
			try
			{
				...
				string fdOreder2 = "Level";
				if (order == 1)
				{
					fdOreder2 = "DutyName";
				}
				fdOreder2 += ",DutyID ";
				foreach (DataRow row in GetPage("Consortia_Duty", queryWhere, page, size, "*", fdOreder2, "DutyID", ref total).Rows)
				{
		...
		public ConsortiaInviteUserInfo[] GetConsortiaInviteUserPage(int page, int size, ref int total, int order, int userID, int inviteID)
		{
			...
			try
			{
				...
				fdOreder2 += ",ID ";
				foreach (DataRow row in GetPage("V_Consortia_Invite", queryWhere, page, size, "*", fdOreder2, "ID", ref total).Rows)
				{
