package labyrinth.view
{
   ...
   public class CleanOutContentItem extends Sprite implements IListCell
   {
      ...
      private function updateItem() : void
      {
         ...
         if(!this._info)
         {
            _loc3_ = LabyrinthManager.Instance.model.currentFloor == 0?int(LabyrinthManager.Instance.model.currentFloor + 1):int(LabyrinthManager.Instance.model.currentFloor);
         ...
         {
            _loc1_ = _loc1_ + ("," + ItemManager.Instance.getTemplateById(this._info.TemplateIDs[_loc2_]["TemplateID"]).Name);
            if(this._info.TemplateIDs[_loc2_]["num"] != 0)
            {
               _loc1_ = _loc1_ + ("x" + this._info.TemplateIDs[_loc2_]["num"].toString());
            }
            _loc2_++;
         }
         if(this._info.HardCurrency > 0)
         {
            _loc1_ = _loc1_ + LanguageMgr.GetTranslation("dt.labyrinth.CleanOutContentItem.HardCurrency",this._info.HardCurrency);
         }
         this._expNum.text = this._expNum.text + _loc1_;
      }
