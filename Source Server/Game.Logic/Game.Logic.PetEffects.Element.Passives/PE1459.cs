...
namespace Game.Logic.PetEffects.Element.Passives
{
	public class PE1459 : BasePetEffect
	{
		...
		{
			PE1459 pE = living.PetEffectList.GetOfType(ePetEffectType.PE1459) as PE1459;
			if (pE == null)
			{
		...
		private void Player_AfterKilledByLiving(Living living, Living target, int damageAmount, int criticalAmount)
		{
			...
			{
				target.AddPetEffect(new CE1459(0, m_probability, m_type, m_currentId, m_delay, base.ElementInfo.ID.ToString()), 0);
			}
