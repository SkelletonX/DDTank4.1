...
namespace Road.Flash
{
	public class FlashUtils
	{
		...
		{
			return new XElement("Item", new XAttribute("StrengthenLevel", info.StrengthenLevel), new XAttribute("Rock", info.Rock));
		}
		...
		public static XElement CreateActiveInfo(ActiveInfo info)
		{
			...
			int num = 5;
			XName name2 = "EndDate";
			string value = (!info.EndDate.HasValue) ? "" : info.EndDate.Value.ToString("yyyy-MM-dd HH:mm:ss");
			XAttribute xAttribute = (XAttribute)(array[num] = new XAttribute(name2, value));
			array[6] = new XAttribute("IsOnly", info.IsOnly);
			int num2 = 7;
			XName name3 = "StartDate";
			string value2 = string.IsNullOrEmpty(info.StartDate.ToString()) ? "" : info.StartDate.ToString("yyyy-MM-dd HH:mm:ss");
			XAttribute xAttribute2 = (XAttribute)(array[num2] = new XAttribute(name3, value2));
			array[8] = new XAttribute("Title", (info.Title == null) ? "" : info.Title);
			array[9] = new XAttribute("Type", info.Type);
			array[10] = new XAttribute("ActiveType", "0");
			array[11] = new XAttribute("IsAdvance", false);
			array[12] = new XAttribute("GoodsExchangeTypes", "");
			array[13] = new XAttribute("GoodsExchangeNum", "");
			array[14] = new XAttribute("limitType", "");
			array[15] = new XAttribute("limitValue", "");
			array[16] = new XAttribute("ActionTimeContent", (info.ActionTimeContent == null) ? "" : info.ActionTimeContent);
