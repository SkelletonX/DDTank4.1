...
namespace Game.Logic.PetEffects.Element.Actives
{
	public class AE1562 : BasePetEffect
	{
		...
		{
			AE1562 aE = living.PetEffectList.GetOfType(ePetEffectType.AE1562) as AE1562;
			if (aE == null)
			{
		...
		private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
		{
			if (IsTrigger)
			{
				...
				target.AddPetEffect(new CE1562(2, m_probability, m_type, m_currentId, m_delay, base.ElementInfo.ID.ToString()), 0);
			}
		}
