...
namespace Game.Logic.PetEffects.Element.Actives
{
	public class AE1367 : BasePetEffect
	{
		...
		{
			AE1367 aE = living.PetEffectList.GetOfType(ePetEffectType.AE1367) as AE1367;
			if (aE == null)
			{
		...
		private void Player_PlayerBuffSkillPet(Player player)
		{
			if (player.PetEffects.CurrentUseSkill == m_currentId)
			{
				foreach (Living item in player.Game.Map.FindAllNearestSameTeam(player.X, player.Y, 250.0, player))
				{
					...
					item.AddPetEffect(new CE1367(4, m_probability, m_type, m_currentId, m_delay, base.ElementInfo.ID.ToString()), 0);
				}
			}
		}
