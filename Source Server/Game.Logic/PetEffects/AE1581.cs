...
{
	public class AE1581 : BasePetEffect
	{
		...
		public AE1581(int count, int probability, int type, int skillId, int delay, string elementID)
			: base(ePetEffectType.AE1581, elementID)
		{
			m_count = count;
			m_coldDown = count;
		...
		{
			AE1581 aE = living.PetEffectList.GetOfType(ePetEffectType.AE1581) as AE1581;
			if (aE == null)
			{
		...
		private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
		{
			if (IsTrigger)
			{
				...
				new CE1581(2, m_probability, m_type, m_currentId, m_delay, base.ElementInfo.ID.ToString()).Start(target);
			}
		}
