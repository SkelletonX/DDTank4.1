   ...
   public class PetsFormView extends Sprite implements Disposeable
   {
      ...
      private function setItemInfo() : void
      {
         ...
         while(_loc3_ < this._petsVec.length)
         {
            ...
            {
               _loc1_ = _loc1_ + this._petsVec[_loc3_].info.HeathUp;
               _loc2_ = _loc2_ + this._petsVec[_loc3_].info.DamageReduce;
            }
            _loc3_++;
         }
      ...
      protected function __onPetsFollowOrCall(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(_loc4_ != -1)
         {
            ...
            this.resetItemInfo(_loc4_,!!_loc3_?int(2):int(1),_loc5_);
            PlayerManager.Instance.Self.PetsID = !!_loc3_?int(_loc4_):int(-1);
            if(_loc3_)
            {
