package calendar.view
{
   ...
   public class ActivityDetail extends Sprite implements Disposeable
   {
      ...
      private static function calculateLast(param1:Date, param2:Date) : String
      {
         ...
         {
            _loc4_ = _loc4_ + (Math.floor(_loc3_ / TimeManager.DAY_TICKS) + LanguageMgr.GetTranslation("day"));
            _loc3_ = _loc3_ % TimeManager.DAY_TICKS;
         }
         if(_loc3_ >= TimeManager.HOUR_TICKS)
         {
            _loc4_ = _loc4_ + (Math.floor(_loc3_ / TimeManager.HOUR_TICKS) + LanguageMgr.GetTranslation("hour"));
            _loc3_ = _loc3_ % TimeManager.HOUR_TICKS;
         }
         else if(_loc4_.length > 0)
         {
            _loc4_ = _loc4_ + ("00" + LanguageMgr.GetTranslation("hour"));
         }
         if(_loc3_ >= TimeManager.Minute_TICKS)
         {
            _loc4_ = _loc4_ + (Math.floor(_loc3_ / TimeManager.Minute_TICKS) + LanguageMgr.GetTranslation("minute"));
            _loc3_ = _loc3_ % TimeManager.Minute_TICKS;
         }
         else if(_loc4_.length > 0)
         {
            _loc4_ = _loc4_ + ("00" + LanguageMgr.GetTranslation("minute"));
         }
         if(_loc3_ >= TimeManager.Second_TICKS)
         {
            _loc4_ = _loc4_ + (Math.floor(_loc3_ / TimeManager.Second_TICKS) + LanguageMgr.GetTranslation("second"));
         }
         else if(_loc4_.length > 0)
         {
            _loc4_ = _loc4_ + ("00" + LanguageMgr.GetTranslation("second"));
         }
         return _loc4_;
      }
