...
{
	public class AE1599 : BasePetEffect
	{
		...
		public AE1599(int count, int probability, int type, int skillId, int delay, string elementID)
			: base(ePetEffectType.AE1599, elementID)
		{
			m_count = count;
			m_coldDown = count;
		...
		{
			AE1599 aE = living.PetEffectList.GetOfType(ePetEffectType.AE1599) as AE1599;
			if (aE == null)
			{
		...
		private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
		{
			if (IsTrigger)
			{
				...
				target.AddPetEffect(new CE1599(2, m_probability, m_type, m_currentId, m_delay, base.ElementInfo.ID.ToString()), 0);
			}
		}
