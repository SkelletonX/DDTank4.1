...
{
	public class AE1583 : BasePetEffect
	{
		...
		public AE1583(int count, int probability, int type, int skillId, int delay, string elementID)
			: base(ePetEffectType.AE1583, elementID)
		{
			m_count = count;
			m_coldDown = count;
		...
		{
			AE1583 aE = living.PetEffectList.GetOfType(ePetEffectType.AE1583) as AE1583;
			if (aE == null)
			{
		...
		private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
		{
			if (IsTrigger)
			{
				...
				new CE1583(2, m_probability, m_type, m_currentId, m_delay, base.ElementInfo.ID.ToString()).Start(target);
			}
		}
