...
{
	public class AE1582 : BasePetEffect
	{
		...
		public AE1582(int count, int probability, int type, int skillId, int delay, string elementID)
			: base(ePetEffectType.AE1582, elementID)
		{
			m_count = count;
			m_coldDown = count;
		...
		{
			AE1582 aE = living.PetEffectList.GetOfType(ePetEffectType.AE1582) as AE1582;
			if (aE == null)
			{
		...
		private void Player_AfterKillingLiving(Living living, Living target, int damageAmount, int criticalAmount)
		{
			if (IsTrigger)
			{
				...
				new CE1582(2, m_probability, m_type, m_currentId, m_delay, base.ElementInfo.ID.ToString()).Start(target);
			}
		}
