      ...
      private function checkProgress() : Boolean
      {
         ...
         for each(_loc3_ in this._gameInfo.roomPlayers)
         {
            if(!_loc3_.isViewer)
            {
               ...
               {
                  this._unloadedmsg = this._unloadedmsg + (_loc3_.playerInfo.NickName + "gameplayer.character.completed false\n");
                  this._unloadedmsg = this._unloadedmsg + _loc7_.character.getCharacterLoadLog();
               }
               _loc1_++;
               ...
               {
                  this._unloadedmsg = this._unloadedmsg + (_loc3_.playerInfo.NickName + "gameplayer.movie.completed false\n");
               }
               _loc1_++;
                  ...
                  for each(_loc9_ in _loc8_.equipdSkills)
                  {
                     if(_loc9_ > 0)
                     {
                        ...
                        if(_loc10_.EffectPic)
                        {
                           ...
                           {
                              this._unloadedmsg = this._unloadedmsg + ("ModuleLoader.hasDefinition(skill.EffectClassLink):" + _loc10_.EffectClassLink + " false\n");
                           }
                           _loc1_++;
                        }
                        if(_loc10_.NewBallID != -1)
                        {
                           ...
                           {
                              this._unloadedmsg = this._unloadedmsg + ("BallManager.findBall(skill.NewBallID):" + _loc10_.NewBallID + "false\n");
                           }
                           _loc1_++;
                        }
                     }
         ...
         while(_loc5_ < this._gameInfo.neededMovies.length)
         {
            if(this._gameInfo.neededMovies[_loc5_].type == 2)
            {
               ...
               {
                  this._unloadedmsg = this._unloadedmsg + ("ModuleLoader.hasDefinition(_gameInfo.neededMovies[i].classPath):" + this._gameInfo.neededMovies[_loc5_].classPath + " false\n");
               }
               _loc1_++;
            }
         ...
         {
            this._unloadedmsg = this._unloadedmsg + "_gameInfo.loaderMap.completed false\n";
         }
         _loc1_++;
